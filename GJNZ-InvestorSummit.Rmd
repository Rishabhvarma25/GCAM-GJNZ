...
prj <- addScenario(conn, proj = 'GJNZ_NZ50_02July.dat', scenario =  c('NZ50_Trn_prcElas0.25'), queryFile = queryFileInput,warn.empty = T,clobber = T) # This command runs queries from queryFileInput against the database, extract the results for two scenario called and write the results to a file called "GJNZ.dat". 
...
prj <- loadProject(proj = "GJNZ_BAU_02July.dat")
```
...
elec <- getQuery(prj,"final energy consumption by aggregate sector and fuel") %>% 
  ...
  group_by(scenario,sector,year) %>% 
    summarise(value=sum(value)) %>%  mutate(scenario="NZ70") %>% 
  group_by(year) %>% 
  mutate(percent=value/sum(value),percent=percent*100)
...
ggplot()+
geom_area(fe %>% filter(year>2015,year<2075,scenario=="NZ70"),mapping=aes(x=year,y=value,fill=input))+
  
  facet_wrap(~scenario) +
  theme_pander(boxes = T,base_size = 14)+
  scale_fill_viridis_d()+
  labs(y="Final Energy Consumption mix (EJ)",x="")+
scale_y_continuous(breaks = seq(0, 20, by = 2), limits = c(0,16))
...
scale_y_continuous(breaks = seq(0, 20, by = 2), limits = c(0,16.5))
...
trans <- getQuery(prj,query = "transport service output by mode") %>% 
  ...
  group_by(scenario,year,mode,category) %>% 
    summarise(value=sum(value)) %>% 
  ungroup()  %>% mutate(scenario="NZ-Ind-2070_GJ-2050_ROW-2080")
...
ggplot()+
geom_area(freight_data %>% filter(year>2015,!mode %in% c("Ship Domestic","Ship International","Aviation")),mapping=aes(x=year,y=value/1000,fill=mode,group=interaction(mode,scenario)))+
...
trans_fuel <- getQuery(prj,"transport final energy by fuel") %>% 
  filter(region=="GJ") %>% mutate(scenario="NZ-Ind-2070_GJ-2050_ROW-2080")
...
build <- getQuery(prj,"building final energy by service (Cooking) and fuel") %>%   filter(region=="GJ",sector=="Cooking") %>% 
  group_by(scenario,input,year) %>% 
  summarise(value=sum(value))%>% 
   mutate(scenario="NZ-Ind-2070_GJ-2050_ROW-2080")
